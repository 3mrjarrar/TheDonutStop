begin;
-- Persist first entry independently of current quantity and the paginated audit log.
alter table public.branch_inventory add column stock_initialized boolean not null default false;
update public.branch_inventory i set stock_initialized = true
where i.quantity > 0 or exists (
  select 1 from public.inventory_events e
  where e.branch_id = i.branch_id and e.variant_id = i.variant_id
    and (e.action in ('restock', 'count') or e.delta > 0)
);
...
begin
  ...
  if length(coalesce(p_reason, '')) > 500 or
     ((stock.stock_initialized or exists (select 1 from public.product_variants v join public.products p on p.id = v.product_id where v.id = p_variant and p.category <> 'donuts')) and length(trim(coalesce(p_reason, ''))) < 2) then
    raise exception 'A reason is required after the first stock entry (2–500 characters)' using errcode = '22023';
  end if;
  ...
  if p_action in ('restock','waste','count') then
    if not exists (select 1 from public.product_variants v
      ...
      where v.id = p_variant and p.category = 'donuts') then
      raise exception 'Drinks support availability changes only';
    end if;
  ...
  update public.branch_inventory set quantity = next_quantity, manual_unavailable = next_unavailable,
    stock_initialized = stock.stock_initialized or (p_action in ('restock', 'count') and next_quantity > 0)
  ...
  insert into public.inventory_events
    ...
    values (p_branch, p_variant, auth.uid(), next_quantity-stock.quantity, coalesce(nullif(trim(p_reason), ''), 'إدخال أولي'),
