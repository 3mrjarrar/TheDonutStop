...
export default function Admin() {
  ...
  const orderSound = useOrderSound(session?.user.id);
  ...
  return <main className={`admin-shell${session && profile && !recovery ? ' admin-dashboard' : ''}`} dir="rtl">
    ...
    <>
      ...
      {profile && branch && <div hidden={section !== 'orders'}><Orders key={branch} sound={orderSound} branch={branch} branches={branches} role={profile.role} onOrders={handleOrders} onChange={() => setRevision(value => value + 1)} /></div>}
