  ...
  return <main className="admin-shell" dir="rtl">
    ...
    {!ready ? <p role="status">جارٍ التحميل…</p> : recovery ? <form className="admin-panel admin-login" onSubmit={changePassword}><h2>تعيين كلمة مرور جديدة</h2><label>كلمة المرور الجديدة<input name="password" type="password" autoComplete="new-password" required minLength={12} /></label><button disabled={busy}>حفظ كلمة المرور</button></form> : !session ? <form className="admin-panel admin-login" onSubmit={login}>
      ...
      {loading ? <p role="status">جارٍ تحميل المخزون…</p> : profile && branch && <>
        <section className="admin-panel"><h2>مخزون {selected?.name_ar}</h2><div className="admin-table-wrap"><table><thead><tr><th>الصنف</th><th>الكمية</th><th>التوفر</th><th>تعديل</th></tr></thead><tbody>{rows.filter(row => row.product_variants.products.name.toLowerCase().includes(search.toLowerCase())).sort((a,b) => a.product_variants.products.name.localeCompare(b.product_variants.products.name)).map(row => <tr key={row.variant_id}><td>{row.product_variants.products.name} {row.product_variants.size !== 'standard' && `(${row.product_variants.size})`}</td><td>{row.quantity}</td><td>{!row.carried ? 'غير مدرج' : row.quantity > 0 && !row.manual_unavailable ? 'متوفر' : 'غير متوفر'}</td><td><button disabled={busy} onClick={() => setEdit({ row, action: 'restock', requestId: crypto.randomUUID() })}>تعديل المخزون</button></td></tr>)}</tbody></table></div>{!rows.length && <p>لا توجد أصناف. تحقق من تشغيل ملف المنيو.</p>}</section>
        {edit && <section ref={editPanel} tabIndex={-1} className="admin-panel" aria-label="تعديل المخزون"><form onSubmit={save} key={edit.requestId}><h2>{edit.row.product_variants.products.name} — {selected?.name_ar}</h2><p>الكمية الحالية: {edit.row.quantity}</p><fieldset disabled={busy}><label>نوع التعديل<select value={edit.action} onChange={event => setEdit(current => ({ ...current, action: event.target.value, requestId: crypto.randomUUID() }))}>{Object.entries(actions).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>{['restock','waste','count'].includes(edit.action) && <label>{edit.action === 'count' ? 'العدد الفعلي بعد الجرد' : 'الكمية'}<input name="amount" type="number" min={edit.action === 'count' ? 0 : 1} max={1000000} step="1" required /></label>}<label>سبب التعديل<input name="reason" minLength={2} maxLength={500} required /></label><p>إضافة المخزون تعيد إتاحة الصنف تلقائيًا.</p><button>حفظ</button> <button type="button" className="secondary" onClick={() => setEdit(null)}>إلغاء</button></fieldset></form></section>}
        <section className="admin-panel"><h2>آخر 30 تعديلًا</h2>{events.length ? <ul className="admin-history">{events.map(item => <li key={item.id}><strong>{rows.find(row => row.variant_id === item.variant_id)?.product_variants.products.name || 'صنف'}</strong> — {actions[item.action] || item.action}: <b dir="ltr">{item.delta > 0 ? '+' : ''}{item.delta}</b> — {item.reason}<small>{new Date(item.created_at).toLocaleString('ar')} · بواسطة {item.actor_id === session.user.id ? profile.name : item.actor_id}</small></li>)}</ul> : <p>لم تُسجّل تعديلات بعد.</p>}</section>
      </>}
    </>}
  </main>;
}
