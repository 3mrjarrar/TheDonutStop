...
let channelId = 0;
export function watchOffers(branchId, refresh) {
  if (!supabase) return () => {};
  const channel = supabase.channel(`offers-${++channelId}`)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'branch_offers', ...(branchId ? { filter: `branch_id=eq.${branchId}` } : {}) }, refresh)
...
export function useOffers(branchId = null) {
  ...
  useEffect(() => {
    ...
    load();
    const unwatch = watchOffers(branchId, load);
